...
if [ ! -d "$DESKTOP" ]; then
    ...
    if [ ! -d "$DESKTOP" ]; then
        ...
        echo -e "\033[31m HATA: Masaüstü dizini bulunamadı Ayarlar.sh dosyasının içindeki DESKTOP(5) değer kısmını masaüstü konumunuzu işaretleyecek şekilde değiştirin. \033[0m"
