...
if [ ! -d "$DESKTOP" ]; then
    ...
    if [ ! -d "$DESKTOP" ]; then
        ...
        echo -e "\033[31mHATA: Masaüstü dizini bulunamadı ~/USM-S/opaklik_ayarla.sh dosyasının içindeki DESKTOP(4) değer kısmını masaüstü konumunuzu işaretleyecek şekilde değiştirin. \033[0m"
