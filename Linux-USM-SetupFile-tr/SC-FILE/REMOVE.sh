...
if [ ! -d "$DESKTOP" ]; then
    ...
    if [ ! -d "$DESKTOP" ]; then
         ...
         zenity --info --title="Desktop Error" --text="HATA: Masaüstü dizini bulunamadı USMKaldır.sh dosyasının içindeki DESKTOP(5) değer kısmını masaüstü konumunuzu işaretleyecek şekilde değiştirin."
